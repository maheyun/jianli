...
ROOT_DIR = os.path.dirname(BASE_DIR)  # 项目根目录（可视化/ 的上级）
...
_conn = sqlite3.connect(os.path.join(ROOT_DIR, '项目一：公司日常运营指标分析', 'ecommerce_ops.db'))
...
_conn = sqlite3.connect(os.path.join(ROOT_DIR, '项目二：老用户激活与价值提升', 'user_activation.db'))
...
_conn = sqlite3.connect(os.path.join(ROOT_DIR, '项目三：各平台ROI预算重新分配', 'roi_allocation.db'))
...
_conn = sqlite3.connect(os.path.join(ROOT_DIR, '项目四：产品组合分析', 'product_portfolio.db'))
